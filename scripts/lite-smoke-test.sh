...
image=${1:-easyimage:ci}
container_name=${LITE_SMOKE_CONTAINER_NAME:-easyimage-lite-smoke-$$-$RANDOM}
...
tmp_root=$(mktemp -d /tmp/easyimage-lite-smoke.XXXXXX)
...
cleanup() {
    ...
    if [[ -n $container_id ]]; then
        container_run_id=$(docker inspect --format '{{index .Config.Labels "com.easyimage.lite-smoke"}}' "$container_id" 2>/dev/null || true)
    ...
    if [[ $safe_to_clean == 1 && $tmp_root == /tmp/easyimage-lite-smoke.* ]]; then
...
container_id=$(docker run --detach --name "$container_name" \
    --label "com.easyimage.lite-smoke=$run_id" \
...
for _ in $(seq 1 20); do
    setup_token=$(docker logs "$container_id" 2>&1 | sed -n 's/.*EasyImage Lite setup token (expires in 30 minutes): \([a-f0-9]\{64\}\).*/\1/p' | tail -n 1 || true)
    [[ -n $setup_token ]] && break
    sleep 0.25
done
