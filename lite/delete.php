...
<!doctype html>
<html lang="zh-CN">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="referrer" content="no-referrer"><meta name="robots" content="noindex,nofollow"><title>删除图片 · EasyImage Lite</title><link rel="stylesheet" href="<?= liteEscape(liteAppUrl($liteConfig, 'assets/app.css')) ?>"></head>
<body class="login-page"><main class="delete-shell"><section class="login-panel"><p class="eyebrow">EasyImage Lite / 删除凭证</p><h1><?= $message !== '' ? liteEscape($message) : '删除这张图片？' ?></h1><?php if ($message === ''): ?><?php if ($valid): ?><p class="intro">此操作不可撤销。删除凭证将在 <?= liteEscape((new DateTimeImmutable('@' . $expires))->setTimezone(new DateTimeZone((string) $liteConfig['timezone']))->format('Y-m-d H:i:s')) ?> 失效。</p><form method="post"><button class="button primary" type="submit">确认删除</button></form><?php else: ?><p class="flash error" role="alert">删除链接无效或已过期</p><?php endif; ?><?php endif; ?><a class="back-link" href="<?= liteEscape(liteAppUrl($liteConfig)) ?>">返回工作台</a></section></main></body>
</html>
